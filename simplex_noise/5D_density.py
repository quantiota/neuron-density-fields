...
def generate_5d_density_field(grid_size=(15, 15, 15, 15, 15), base_density=10000000, 
    ...
    """
    Generate a 5D neuron density field using Simplex noise.

    Args:
        grid_size: Tuple (nx, ny, nz, nw, nq) defining the 5D grid dimensions
        base_density: Base neuron density (neurons per mm^5)
    ...
    Returns:
        torch.Tensor: 5D tensor of neuron counts per grid cell
    """
    nx, ny, nz, nw, nq = grid_size
    ...
    q = torch.linspace(0, nq * scale, nq)
    ...
    for i in range(nx):
        for j in range(ny):
            for k in range(nz):
                for l in range(nw):
                    for m in range(nq):
                        noise_val = snoise5(x[i].item(), y[j].item(), z[k].item(), w[l].item(), q[m].item(),
                                           octaves=octaves,
                                           persistence=persistence,
                                           lacunarity=lacunarity)
                        density[i, j, k, l, m] = base_density * (1 + density_variation * noise_val)
    
    # Calculate volume of each grid cell (in 5D mm^5)
    cell_volume = scale ** 5
...
def visualize_5d_density(neurons_per_cell, grid_size, scale, base_density=10000000):
    """
    Visualize the 5D neuron density field with 2D slices.

    Args:
        neurons_per_cell: 5D tensor of neuron counts
        grid_size: Tuple (nx, ny, nz, nw, nq) of grid dimensions
        scale: Physical size of each grid cell in mm
        base_density: Base neuron density (neurons per mm^5) for statistics display
    """
    nx, ny, nz, nw, nq = grid_size
    volume_mm5 = nx * ny * nz * nw * nq * (scale ** 5)
    
    # 2D Slice Visualization at z=7, w=7, q=7
    slice_z = 7
    slice_w = 7
    slice_q = 7
    ...
    # Main heatmap
    ax = plt.subplot(111)
    im = ax.imshow(neurons_per_cell[:, :, slice_z, slice_w, slice_q].detach().cpu().numpy(), cmap='plasma', 
    ...
    ax.set_title(f'Neuron Density at z = {slice_z}, w = {slice_w}, q = {slice_q} (5D)', fontsize=14, pad=15, y=1.02)
    ...
    stats_text = (
        f"Grid size: {nx}×{ny}×{nz}×{nw}×{nq} cells\n"
        f"Physical 5D volume: {volume_mm5:.1f} mm⁵\n"
        f"Cell size: {scale}×{scale}×{scale}×{scale}×{scale} mm\n"
    ...
    plt.tight_layout()
    plt.savefig('density_xd_results/neuron_density_5D.png', dpi=300, bbox_inches='tight')
    plt.close()
    
    # 3D Scatter Visualization (showing a 3D slice of the 5D space)
    fig = plt.figure(figsize=(12, 10))
    ax = fig.add_subplot(111, projection='3d')
    
    # Create grid coordinates for a 3D slice of the 5D space
    ...
    density_values = neurons_per_cell[:, :, :, slice_w, slice_q].flatten().detach().cpu().numpy()
    ...
    ax.set_title(f'3D Slice of 5D Neuron Density Field (w = {slice_w}, q = {slice_q})')
    
    plt.tight_layout()
    plt.savefig('density_xd_results/density_5d_scatter.png')
    plt.close()

def main():
    # Parameters (updated for 5D)
    grid_size = (15, 15, 15, 15, 15)
    base_density = 10000000  # 10 million neurons per mm^5
    ...
    # Generate density field
    neurons_per_cell = generate_5d_density_field(
        grid_size=grid_size,
    ...
    visualize_5d_density(neurons_per_cell, grid_size, scale, base_density=base_density)
    print("5D Density field generated and visualizations saved in 'density_3d_results' directory.")
